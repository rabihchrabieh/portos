 ...
 *   po_priority(P)
 *
 *   NO LONGER PROCESSES po_signal and po_time directives
 *   po_signal(S,G), po_signal(S,G,H), po_signalp
 *   po_time(T,C), po_time(T,C,H), po_timep
 *
...
#if 0
//#define CODEWORD 0x18dd6b0b    // DEBUG version
#define CODEWORD 0x5ab1324b
#endif

// Check version
int Checkver = 0xa3f84b7e;
int Checkmode = 0;

/* CODE
 */
static char *Code = NULL;
static char *CodeEnd = NULL;
...
static void outputCode(void)
{
  ...
  while ( 1 ) {
	...
	break;
      case 'B': // Start block for arguments (zero or more as needed)
	if ( PF->nargs == 0 ) ignore = 1;
	else c_bracket = c;
	arg = 0;
	break;
      case 'b': // End block for arguments (zero or more as needed)
	ignore = 0;
    ...
    if ( c != c0 ) {
      if ( c[-1] != '\r' ) fwrite(c0, 1, c-c0, Fout);
      else if ( c-c0 > 1 ) fwrite(c0, 1, c-c0-1, Fout);
    }

    c++;
    if ( c - CodeEnd >= 0 ) break;

    if ( CR && c[-1] != '\r' ) fprintf(Fout, "\r\n");
    else fprintf(Fout, "\n");
...
static void SMlevel(int position)
{
  ...
  case '#':  // Preprocessor
	...
	else if ( Substate == '\'' && c[0] == '\'' ) Substate = 0; // VERIFY
...
void SMmain(void)
{
  ...
  #if 0
  int insideTemplate = 0, codeWord = 0;
  int versionProblem = 0;
  #endif
  ...
  for ( position = 0 ; position < InputLen ; position++ ) {
    ...
    if ( Code == NULL ) {
      #if 0
      if ( insideTemplate && isgraph(Input[position]) ) {
	codeWord += Input[position] << ((position-insideTemplate) & 0x1F);
      }
      #endif
    ...
    #if 0
    versionProblem = (codeWord != CODEWORD);
    #endif
    ...
    // Fix po_priority(), po_time(), po_signal()
    ...
    // CANCELED: We no longer process po_signal and po_time
    if ( 0 && Level >= 1 && SymbolEndStrobe &&
