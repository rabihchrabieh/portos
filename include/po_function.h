...
#ifndef po_function__H
#define po_function__H
...
typedef struct _po_function_ServiceHandle po_funcSrvH;
...
typedef struct _po_function_ServiceHandle {
  po_function_Service *service; /* MUST BE FIRST (inherited and overlapped) */
...
/* Structure for bitmap's linked list
 */
typedef struct {
  #if po_DEBUG
  // This is to track functions
  po_function_Handle * volatile first_tmp;
  #endif
  po_function_Handle * volatile last;
  po_function_Handle * volatile first;
} po_function_BitmapList;
...
typedef struct _po_function_Environ {
  ...
  po_function_BitmapList list[po_function_NUM_PRI_LEVELS];
...
/* Report scheduled priority functions
 */
void po_function_reportsched_(int priority);
static inline void po_function_reportsched(int priority)
{
  #if po_TEST
  po_function_reportsched_(priority);
  #endif
}
...
/*-GLOBAL-
 * Resume priority functions after hardware or software interrupt.
 */
static inline void po_function_resume(void)
{
  po_function_restorepri(po_function_Env.currpri);
}
 ...
 * calls to priority functions inside HWI should be wrapped with HWI_enter
 * and HWI_exit.
 */
static inline void po_function_exithwi(void)
{
  ...
  // by any preempting HWI
  int currpri = po_function_Env.currpri - (po_function_NUM_PRI_LEVELS+1);
  po_function_Env.currpri = currpri;
  if ( po_function_Env.maxpri > currpri ) po_function_context();
}
